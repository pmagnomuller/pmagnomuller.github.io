...
While the Claude plan still has capacity, every request goes to Anthropic as usual. Conduit watches for real plan-quota signals, not every random `429`. Only the ones that mean *your usage window is done*. When that fires, it opens a circuit breaker and transparently replays the request on a fallback provider.
